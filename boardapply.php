...
$userid = addslashes($obj->userid);
$username = addslashes($obj->username);
$date = addslashes($obj->date);
$starttime = addslashes($obj->starttime);
$stmt = $conn->prepare("INSERT INTO $obj->table(boardtitle,boardcontent,filelists,userid,username,date,starttime)
VALUES ('$boardtitle','$boardcontent','$filelists','$userid','$username','$date','$starttime')");
$stmt->execute();
$sql = "select *from board where boardtitle ='$boardtitle' and boardcontent ='$boardcontent'
and filelists='$filelists' and userid ='$userid' and username ='$username' and date ='$date'";
$res = $conn->query($sql);
$row=mysqli_fetch_array($res);
$sql2 = "select *from boarduploadfile where starttime ='".$row['starttime']."' and userid='".$userid."' and type='M'";
