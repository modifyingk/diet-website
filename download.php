...
if(isset($_GET['filepath'])){ //Get 방식으로 얻어온 파일 경로 정보가 있다면
    ...
    echo "<script>location.href='/test.php';</script>"; //test.php로 이동시킨다.
    exit(); //php문을 종료한다.
}
if(isset($_GET['filename'])){ //Get 방식으로 얻어온 파일 이름이 있다면
    $filename = urlencode($_GET['filename']); //파일 이름을 urlencode 방식으로 저장해준다.
} else {
